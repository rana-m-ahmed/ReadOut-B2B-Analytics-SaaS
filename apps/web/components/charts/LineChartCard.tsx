...
export function LineChartCard({ payload }: ChartProps) {
    ...
    <Card className="w-full h-full min-h-[300px] flex flex-col p-6 border-[var(--hairline)] shadow-sm bg-[var(--surface)]">
      ...
      <div className="flex-1 min-h-[200px]">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={payload.data} margin={{ top: 5, right: 5, left: -20, bottom: 0 }}>
            <XAxis 
              dataKey={xKey} 
              axisLine={false} 
              tickLine={false} 
            ...
            {yKeys.map((key: string, idx: number) => (
              <Line 
                ...
                type="monotone" 
                dataKey={key} 
                stroke={yKeys.length === 1 ? "var(--accent)" : getCategoryColor(key)} 
                strokeWidth={3} 
                dot={false}
                activeDot={{ r: 6, strokeWidth: 0, fill: yKeys.length === 1 ? "var(--accent)" : getCategoryColor(key) }}
              />
            ))}
          </LineChart>
        </ResponsiveContainer>
