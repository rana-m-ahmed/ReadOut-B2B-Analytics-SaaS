...
- Deployment artifacts: complete for the verified Render-first path. `apps/api/requirements.txt` is finalized with exact pinned versions, env docs are cross-checked against `core/config.py`, and fresh-environment `uvicorn` boot plus `/health` response are validated with only documented env vars set. Hugging Face Spaces is not kept as an actively verified fallback artifact because Docker is unavailable in this workspace, so no locally-validated Dockerfile is shipped.
